		...
		<section class="application container">
			...
			<section class="input-block">
				...
				<div class="row">
					<div class="col-6">
						...
						<div class="row info">
							<label>
                                <span class="text-danger">* </span>培训工种</label>
						...
						<div class="row info">
							<label>
                                ...
                                <input v-if="currentClass!=''" :readonly="currentClass!=''" class="form-control col-md-6" v-model="class_obj.TrainingLevel" type="text" name="" id="" value="" />
			...
			<section class="input-block">
				...
				<div class="row info">
					...
					<a href="xlsx/学员花名册.xlsx" class="btn">下载模板</a>
				</div>
    ...
    submitInfo: function() {
      ...
      new Promise((a, b) => {
        if (self.class_obj.StudentUrl != "") {
          console.log(1);
          $.get({
            url: `//114.55.173.248:2000/v1/student/excel?path=${
              self.class_obj.StudentUrl
            }`,
            success(res) {
              var data = JSON.parse(res);
              if (data.Result) {
                self.class_obj.TrainingPersonNumber =
                  data.Memory.DataList.length;
        ...
        .then(data => {
          ...
          if (self.class_obj.Status == 1) {
            ...
            ajax("post", "/class", JSON.stringify(self.class_obj), function(
              res
            ) {
